...
internal func exerciseOne() {
    ...
    print("\(firstName) \(lastName) lives at \(streetName) in \(city), \(state), \(postCode). If you want to contact \(title). \(lastName), you can email \(emailAdress) or call at \(cellNumber).")
...
internal func exerciseThree() {
    ...
    for value in allMovies {
        if value.rightsOwner.contains("Disney") {
            print(value.name)
        }
    }
    ...
    for value in allMovies {
        if value.price < 15.00 {
            print("\(value.name): \(String(value.price))")
        }
    }
